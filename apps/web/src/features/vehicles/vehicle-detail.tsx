...
import { ArrowLeft, Calculator, Clock, RefreshCw, Tag } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex min-h-11 items-center gap-2 rounded-[var(--radius-control)] text-sm font-semibold text-[var(--muted)] transition-colors hover:bg-[var(--surface-hover)] hover:text-[var(--foreground)]"
        ...
        <div>
          ...
          <p className="text-xs text-[var(--muted)]">
            ID: {vehicle.id} • Registrado el{" "}
            {formatDate(vehicle.first_listed_at)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        >
          <h3
            ...
            className="mb-4 text-base font-bold text-[var(--foreground)]"
          >
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            ...
            >
              <RefreshCw
                ...
                size={12}
                className={
                  estimateMutation.isPending ? "animate-spin" : undefined
                }
              />
          ...
          {estimate ? (
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="flex items-baseline gap-3">
                  ...
                  <Badge
                    tone={confidenceBadgeVariant(estimate.confidence_score)}
                  >
              ...
              <div className="grid grid-cols-2 gap-2 rounded-[var(--radius)] bg-[var(--surface)] p-3 text-xs">
                <div>
                  <span className="text-[var(--muted)]">
                    Rango razonable (P25 - P75):
                  </span>
                  <p className="font-semibold text-[var(--foreground)]">
                    {formatPrice(estimate.low_amount, estimate.currency)} –{" "}
                    {formatPrice(estimate.high_amount, estimate.currency)}
                  </p>
                </div>
                <div>
                  <span className="text-[var(--muted)]">
              ...
              <p className="text-[0.75rem] text-[var(--muted)]">
                Calculado el {formatDate(estimate.calculated_at)} mediante
                mediana IQR de comparables homogéneos.
              </p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center gap-3 py-6 text-center">
              ...
              />
              <p className="text-sm text-[var(--muted)]">
                Aún no se ha calculado la estimación de mercado para este
                vehículo.
              </p>
              <Button
                onClick={() => estimateMutation.mutate()}
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 text-center sm:grid-cols-4">
            <div className="rounded-[var(--radius)] border border-[var(--border)] p-3">
              ...
              <p className="mt-1 flex items-center justify-center gap-1 text-xl font-bold text-[var(--foreground)]">
      ...
      >
        ...
        {vehicle.listings.length === 0 ? (
          <p className="text-sm text-[var(--muted)]">
            No hay anuncios vinculados a este vehículo.
          </p>
        ) : (
            ...
            {vehicle.listings.map((l) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-[var(--muted)]">
                    Ref: {l.external_id} {l.province ? `• ${l.province}` : ""} •{" "}

    ...
    return (
      ...
      >
        <p className="text-sm text-[var(--muted)]">
          Cargando cola de deduplicación…
        </p>
      </div>
    );
  }
      ...
      <div className="p-4" role="alert">
        <p className="text-sm font-medium text-[var(--danger)]">
          {error instanceof Error
            ? error.message
            : "Error al cargar candidatos de matching."}
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted)]">
            Revisión manual asistida para unificar anuncios de un mismo vehículo
      ...
      {feedback && (
        <div
          role="status"
          aria-live="polite"
          className="flex items-center justify-between rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--accent-soft)] p-3 text-xs text-[var(--foreground)]"
        >
          <span>{feedback}</span>
          <button
            type="button"
      ...
      {items.length === 0 ? (
        <div className="empty-state">
          ...
          <p className="max-w-md text-sm text-[var(--muted)]">
            No hay candidatos duplicados pendientes de confirmación. Puedes
            ejecutar un escaneo para buscar nuevas coincidencias entre los
            anuncios importados.
          </p>
        </div>
      ) : (
        <div
          ...
          aria-label="Lista de candidatos a emparejar"
        >
          {items.map((candidate) => (
            <CandidateCard
              key={candidate.id}
              ...
              isConfirming={
                ...
                confirmMutation.variables === candidate.id
              }
              isRejecting={
                rejectMutation.isPending &&
                rejectMutation.variables === candidate.id
              }
...
function CandidateCard({
  ...
  return (
    <article className="flex flex-col gap-4 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface-raised)] p-5 shadow-[var(--shadow-card)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] pb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex min-h-10 items-center gap-1.5 rounded-[var(--radius-control)] border border-[color-mix(in_srgb,var(--danger)_25%,transparent)] bg-[var(--danger-soft)] px-3 text-xs font-semibold text-[var(--danger)] transition-colors hover:border-[var(--danger)] disabled:opacity-50"
          ...
          <button
            ...
            className="inline-flex min-h-10 items-center gap-1.5 rounded-[var(--radius-control)] bg-[var(--accent)] px-3 text-xs font-semibold text-white transition-colors hover:bg-[var(--accent-hover)] disabled:opacity-50"
          ...
          {la ? (
            <div className="mt-1 flex flex-col gap-1 text-xs">
              ...
              <p className="font-semibold text-[var(--foreground)]">
                {formatPrice(la.price_amount, la.price_currency)} •{" "}
                {formatKm(la.mileage_km)}
              </p>
        ...
        <div className="rounded-[var(--radius-control)] bg-[var(--surface-inset)] p-3">
          ...
          {lb ? (
            <div className="mt-1 flex flex-col gap-1 text-xs">
              ...
              <p className="font-semibold text-[var(--foreground)]">
                {formatPrice(lb.price_amount, lb.price_currency)} •{" "}
                {formatKm(lb.mileage_km)}
              </p>
              <p className="text-[var(--muted)]">

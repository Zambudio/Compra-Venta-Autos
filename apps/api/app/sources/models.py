...
class SourceConfig(TimestampMixin, Base):
    """Configuración operativa persistida para un conector registrado."""

    __tablename__ = "source_configurations"

    id: Mapped[UUID] = mapped_column(primary_key=True, default=uuid4)
    source_key: Mapped[str] = mapped_column(
        ForeignKey("sources.key", ondelete="CASCADE"), unique=True, nullable=False
    )
...
class SourceConfigChange(Base):
    ...
    id: Mapped[UUID] = mapped_column(primary_key=True, default=uuid4)
    source_key: Mapped[str] = mapped_column(
        ForeignKey("sources.key", ondelete="CASCADE"), nullable=False
    )
